...
public class TollPopup extends JDialog
{
   ...
   {
      // 팝업 틀 설정
      setTitle("요구체력 지불 및 인수하기"); // 팝업 설정 5종
      setSize(500,280);
      setResizable(false);
      ...
      String str1 = String.format("%,d", num1 / 2); //통행료     
      //double num2 = MarbleGame.st[MarbleManager.posi].getPrice() * (MarbleGame.st[MarbleManager.posi].getTakeCount()+1) +50; // 인수비 +50씩 증가
      double num2 = 100; // 인수비 +100씩 증가
      ...
      text[0].setText("현재 \"" + MarbleGame.st[MarbleManager.posi].getName() + "\" 의 요구되는 체력은 " + str1 + "입니다.");
      text[0].setBounds(0, 40, 500, 30);
      text[1].setText("인수할려면 체력 " + str2 + "이 필요합니다.");
               ...
               if(MarbleGame.p[MarbleManager.pnum].getMoney() < num1) // 현금이 통행료보다 적을 경우
               {
                  if(MarbleGame.p[MarbleManager.pnum].getProperty() < num1) // 전재산이 통행료보다 적을 경우
                  {
                     ...
                     for(int i=0;i<3;i++)
                     {
                        ...
                        MarbleGame.st[MarbleGame.pst[MarbleManager.pnum][i]].setToll((int)(MarbleGame.st[MarbleGame.pst[MarbleManager.pnum][i]].getPrice())); // 통행료 초기화
                        MarbleGame.st[MarbleGame.pst[MarbleManager.pnum][i]].setAdmission((int)(MarbleGame.st[MarbleGame.pst[MarbleManager.pnum][i]].getPrice())); // 관람료 초기화
                        MarbleGame.st[MarbleGame.pst[MarbleManager.pnum][i]].setOwnerCheck(0); // 소유 여부 초기화
                        MarbleGame.pst[MarbleManager.pnum][i] = 0; // 카드 제거
                     }

                     JOptionPane.showMessageDialog(MarbleGame.jlp, "체력고갈 되었습니다. 게임에 참여할 수 없습니다.", "상태창", JOptionPane.ERROR_MESSAGE);
                  ...
                  else
                  {
                     ...
                     JOptionPane.showMessageDialog(MarbleGame.jlp, "소유한 과목을 팔아 요구체력을 지불합니다.", "상태창", JOptionPane.ERROR_MESSAGE);
                  }
               }
               else
               {
                  ...
                  JOptionPane.showMessageDialog(MarbleGame.jlp, "이미 소유한 과목이 3개입니다. 요구체력만 지불합니다.", "상태창", JOptionPane.ERROR_MESSAGE);
               }
            }
            else
            {
               if(MarbleGame.p[MarbleManager.pnum].getMoney() < num1+num3) // 현금이 통행료+인수비보다 적을 경우
               {
                  ...
                  else
                  {
                     ...
                     JOptionPane.showMessageDialog(MarbleGame.jlp, "체력이 부족하여 인수할 수 없습니다. 요구체력만 지불되었습니다.", "상태창", JOptionPane.INFORMATION_MESSAGE);
                  }
               }
               else
               {
      ...
      no.addActionListener(new ActionListener() {
         public void actionPerformed(ActionEvent e) {
            ...
            else
            {
               ...
               JOptionPane.showMessageDialog(MarbleGame.jlp, "요구체력이 정상 지불되었습니다.", "상태창", JOptionPane.INFORMATION_MESSAGE);
            }
